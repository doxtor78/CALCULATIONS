...
KRAKEN_API_KEY = 'REMOVED'
KRAKEN_API_SECRET = 'REMOVED=='
...
def calculator():
    if request.method == 'POST':
        # Store all form inputs in the session to remember them
        session['calculator_inputs'] = request.form
        ...
        try:
            ...
            inputs = {
                ...
                if k not in ['trade_side', 'capital_type', 'calculation_type', 'save']
            }
            inputs.update({
                k: v for k, v in request.form.items() 
                if k in ['trade_side', 'capital_type', 'calculation_type', 'save']
            })
            ...
            step = (entry_2 - entry_1) / 4
            entry_levels = [entry_1 + (step * i) for i in range(5)]
            ...
            for strat in ['symmetric', 'asymmetric']:
                ...
                if strat == 'symmetric':
                    res['avg_entry'] = (entry_1 + entry_2) / 2
                else: # asymmetric
            ...
            # Add a print statement to see errors in the console during debugging
            print(f"An error occurred: {e}") 
            return render_template('calculator.html', error=str(e), inputs=request.form)

    # For a GET request, load the inputs from the session
    saved_inputs = session.get('calculator_inputs', {})
    return render_template('calculator.html', inputs=saved_inputs)
...
BYBIT_API_KEY = 'REMOVED'
BYBIT_API_SECRET = 'REMOVED'
...
# --- API Credentials ---
COINMARKETCAP_API_KEY = 'REMOVED'

# Bitstamp
BITSTAMP_API_KEY = 'REMOVED'
BITSTAMP_API_SECRET = 'REMOVED'
BITSTAMP_CUSTOMER_ID = 'REMOVED'

# Bitfinex
BITFINEX_API_KEY = 'REMOVED'
BITFINEX_API_SECRET = 'REMOVED'

# BitMEX
BITMEX_API_KEY = 'REMOVED'
BITMEX_API_SECRET = 'REMOVED'
