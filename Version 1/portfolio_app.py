...
def calculator():
    if request.method == 'POST':
        try:
            ...
            inputs = {k: float(v) if k not in ['trade_side', 'capital_type'] else v for k, v in request.form.items()}
            ...
            for i in range(5): # Loop over 5 TPs for exits
                ...
                # Phase 6 (Round Exits)
                p6_rounded_usd = custom_round(p5_volume_usd)
            ...
            return render_template('calculator.html', results=results, inputs=request.form, warning_symmetric=warning_symmetric, warning_asymmetric=warning_asymmetric)
        except (ValueError, KeyError) as e:
            # Handle cases where form values are missing or not numbers
            return render_template('calculator.html', error=f"Invalid input: {e}", inputs=request.form)

    # For GET request, just show the form
    return render_template('calculator.html', inputs={}, warning_symmetric=None, warning_asymmetric=None)
